...
fecha: 2026-05-18
revision: 2026-05-20 (r5) — Spec 41 implementada: sistema completo de coding viento operativo (vault + pipeline + frontend + skill)
tipo: documento-epic
...
- **Spec 42 (capa precipitación)** está en **borrador-r1**, primera capa real diseñada. Cierra 12 decisiones técnicas y deja 3 decisiones visuales abiertas (UI de subindicadores, distinción crecimiento/recesión, glyph SVG) con brief autocontenido en Anexo A para sesión de Product Design. Establece el patrón de subindicadores que las Specs 43-45 heredan, cerrando la pregunta abierta #4 del epic en su dimensión técnica (la dimensión visual queda en Product Design).
...
| 42 | Capa precipitación: crecimiento económico | Spec 39, 40 | **borrador-r1** — 12 decisiones técnicas cerradas; 3 decisiones visuales (UI subindicadores, distinción crecimiento/recesión, glyph) abiertas con brief autocontenido para sesión de Product Design |
...
6. ~~**Diseñar Spec 42 (capa precipitación) en Cowork.**~~ **Hecho 2026-05-20 en borrador-r1.** Cierra 12 decisiones técnicas (escala secuencial intensidad con 5 buckets de magnitud, paleta terracota, separación magnitud/dirección, contrato técnico de subindicadores, cobertura editorial piloto ARG/BRA/CHI con cobertura técnica 10/10). Resuelve la pregunta abierta #4 del epic estableciendo el patrón de subindicadores que las Specs 43-45 heredan. Deja 3 decisiones visuales abiertas (A: UI de subindicadores, B: distinción crecimiento/recesión, C: glyph SVG) con brief autocontenido en Anexo A. **Próxima acción: sesión de Product Design** que cierre A/B/C → Spec 42 bumpea a r2 → handoff a VS Code.
...
4. ~~**¿Subcapas / sub-indicadores por capa? (apertura 2026-05-19)**~~ **Cerrada parcialmente en Spec 42 r1 (2026-05-20).** Dimensión técnica resuelta: sí, el patrón es general y se materializa extendiendo el contrato `Layer` con `subIndicators?: LayerSubIndicator[]` opcional. Spec 42 expone los 4 subindicadores definidos en el epic. Dimensión visual (chips hover / drawer / controles / combinación) queda en sesión de Product Design dedicada — ver Spec 42 Anexo A Decisión A. Cuando Product Design cierre, Spec 42 bumpea a r2 y se extiende contrato `Layer` en r3 de Spec 39.
