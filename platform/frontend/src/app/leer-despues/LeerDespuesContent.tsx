...
interface LeerDespuesContentProps {
  allAnalyses: AnalisisEntry[];
}

export default function LeerDespuesContent({ allAnalyses }: LeerDespuesContentProps) {
  const { state, toggleSaved } = useReaderState();
  const [mounted, setMounted] = useState(false);

  useEffect(() => { setMounted(true); }, []);

  const savedKeys = mounted && state ? new Set(state.saved) : new Set<string>();
  const saved = allAnalyses.filter(
    a => savedKeys.has(`${a.countrySlug}-${a.year}-w${a.week}`)
  );
    ...
    <>
      ...
      <Suspense>
        <ArchivoConFiltros
          ...
          h1Title="Leer después"
          onRemove={toggleSaved}
          emptyMessage="Tu lista está vacía. Marcá análisis con el botón 'Leer después' al final de cada uno."
        />
      </Suspense>
          ...
          {!mounted ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--mi-space-4)" }}>
              {saved.map(a => {
                ...
                const href = isPublication ? `/publicaciones/${a.slug}` : `/analisis/${a.countrySlug}/${a.slug}`;
                return (
                  <div
                    key={a.slug}
                    style={{
